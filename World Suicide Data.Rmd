...
Now, to do some additional data cleaning.

```{r}
#Creates a column ('Max.Age.Group') representing the maximum suicide rate for each country, based on age group.
Combined.Data$Max.Age.Group <- apply(Combined.Data[5:8], 1, max)

#Remove countries with Max.Age.Group==0, since they don't include data based on age group.
Combined.Data <- Combined.Data[!(Combined.Data[,9]=="0"),]

#Creates a column ('Which.Age.Group') representing which age group features the highest suicide rate per 100K citizens.
Combined.Data$Which.Age.Group <- apply(Combined.Data[5:8],1,function(z) names(which.max(z)))
```

Visualizations! First, Plot #1 gives an overview of the relationships between different data.

```{r echo=FALSE, message=FALSE, warning=FALSE}
Plot_1 <- ggplot2::ggplot(Combined.Data) + ggplot2::ylim(1,50) + ggplot2::geom_point(data=Combined.Data,ggplot2::aes(x=GDP.per.Capita,y=Suicide.Rate..per.100K.,colour=Which.Age.Group,size=Population.Size,alpha=I(0.6))) + ggplot2::ggtitle("Suicide Rate (per 100K Citizens) vs. GDP per Capita") + ggplot2::xlab("GDP per Capita") + ggplot2::ylab("Suicide Rate (per 100K Citizens)") + ggplot2::labs(colour="Age Group with Highest Suicide Rate",size="Population Size") + ggplot2::geom_smooth(ggplot2::aes(x=GDP.per.Capita,y=Suicide.Rate..per.100K.,colour=Which.Age.Group),fill=NA) 

plot(Plot_1)
```

Plot #2 is a histogram depicting the proportion of deaths in each age group, as well as the number of countries with each proportion.
